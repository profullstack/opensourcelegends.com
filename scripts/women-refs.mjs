#!/usr/bin/env node
/*
  Women in Tech (Series Five) — reference photo resolution.
  ...
  Provenance for every accepted photo is written to data/women-references.json
  ...
  Run:
    node scripts/women-refs.mjs resolve    # find candidates, write the manifest
    node scripts/women-refs.mjs fetch      # download accepted references
    node scripts/women-refs.mjs report     # who is still missing a face
*/
...
const REFS_DIR = path.join(ROOT, 'assets', 'references', 'women-in-tech');
const MANIFEST = path.join(ROOT, 'data', 'women-references.json');
 ...
 */
const OVERRIDES = {
  // Profile photographs from their own accounts, each verified against the real
  // name the API reports. Not freely licensed, so recorded as 'supplied'.
  // Commons search returned a French astronaut of the same name in a flight suit.
  'camille-fournier': {
    url: 'https://avatars.githubusercontent.com/u/607890?v=4&s=460',
    credit: 'Camille Fournier',
    license: 'supplied',
    sourceUrl: 'https://github.com/skamille',
    note: 'Profile photograph; account verified against their real name.',
  },
  'carol-willing': {
    url: 'https://avatars.githubusercontent.com/u/2680980?v=4&s=460',
    credit: 'Carol Willing',
    license: 'supplied',
    sourceUrl: 'https://github.com/willingc',
    note: 'Profile photograph; account verified against their real name.',
  },
  'sarah-drasner': {
    url: 'https://avatars.githubusercontent.com/u/2281088?v=4&s=460',
    credit: 'Sarah Drasner',
    license: 'supplied',
    sourceUrl: 'https://github.com/sdras',
    note: 'Profile photograph; account verified against their real name.',
  },
  'jessie-frazelle': {
    url: 'https://avatars.githubusercontent.com/u/1445228?v=4&s=460',
    credit: 'Jess Frazelle',
    license: 'supplied',
    sourceUrl: 'https://github.com/jessfraz',
    note: 'Profile photograph; account verified against their real name.',
  },
  'leonie-watson': {
    url: 'https://avatars.githubusercontent.com/u/3501033?v=4&s=460',
    credit: 'Leonie Watson',
    license: 'supplied',
    sourceUrl: 'https://github.com/LJWatson',
    note: 'Profile photograph; account verified against their real name.',
  },
};
 ...
 */
const REJECTED = {
  // Looked at, did not hold up. Reasons recorded so a rerun cannot undo the call.
  'jean-bartik':
    'Wikidata P18 is "Two women operating ENIAC", a wide room shot where no face is legible. Conditioning on it would invent one.',
  'betty-holberton':
    'The only Commons photograph is a small, low-resolution profile at the ENIAC panels. Not enough face to paint a likeness from without guessing.',
  'joan-clarke':
    'Commons search returned a photograph of the house at 193 Rosendale Road that carries her blue plaque. Her Bletchley-era portraits are not freely licensed.',
  'anita-borg':
    'Commons search returned a screenshot of her Wikidata item structure, a diagram.',
  'leonie-watson':
    'Her GitHub avatar is the tink logo and her about page serves no photograph.',
  'julia-evans':
    'Commons search returned "Julia Evans Reed.jpg", a different person, and her GitHub avatar is a cartoon star rather than a photograph.',
  'kathleen-booth':
    'No freely licensed photograph found. Wikipedia carries no portrait.',
};
...
async function loadRoster() {
  // src/data/women.ts statically imports the manifest, so it has to exist
  ...
  const mod = await import(path.join(ROOT, 'src', 'data', 'women.ts'));
  return mod.builders;
}
 ...
 */
const FIELD =
  /comput|software|engineer|scientist|research|professor|technolog|inform|program|mathematic|cryptanal|cryptograph|physicist|invent|designer|activist|entrepreneur|business|academic|actress|writer|analyst/i;
...
function looksRight(entity, pro) {
  ...
  // No birth-year floor either, for the same reason. Ada Lovelace was born in 1815.
...
async function commonsMeta(filename) {
  ...
  // NO DATE GUARD IN THIS SET. Series Three rejected pre-1950 photographs because
  // an old plate meant the search had found a historical namesake. Here the
  // historical figures ARE the subject: Lovelace died in 1852, the ENIAC women were
  // photographed in 1946, and Joan Clarke's Bletchley portraits are wartime. That
  // guard would have silently deleted the first third of the roster.
