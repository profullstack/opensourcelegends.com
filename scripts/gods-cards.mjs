#!/usr/bin/env node
/*
  Gods of AI (Series Four) — end-to-end card pipeline.
  ...
  exists, so re-running is cheap and a failed batch can just be run again.
    ...
    portraits  AI portrait art, one per professional -> assets/portraits-art/gods-of-ai/
    render     HTML template -> exact card PNGs      -> dist/gods/html/
    enhance    image-to-image premium finish pass    -> dist/gods/enhanced/  (OPT-IN, see below)
    publish    pick the best face available          -> public/cards/gods/
  ...
  scripts/gods-refs.mjs), and a card with no verified photo gets NO FACE — the
  ...
  conditioned portrait is a derivative work. data/ai-references.json carries
  the provenance and the card page prints it.

  The roster is src/data/ai.ts, imported directly (Node strips the types),
  so there is no second copy of the data to keep in sync.

  Portrait resolution order, highest authority first:
    1. assets/portraits/gods-of-ai/card_NNN.png   (human-approved, committed)
    2. assets/portraits-art/gods-of-ai/card_NNN.png (model output, gitignored)
  ...
  Run:
    node scripts/gods-cards.mjs all              # whole set
    node scripts/gods-cards.mjs all 1 5 12       # only these numbers
    node scripts/gods-cards.mjs render           # no API calls, no spend
*/
...
import { buildFront, buildBack, CARD_W, CARD_H } from './gods-template.mjs';
...
// Importing src/data/ai.ts makes Node warn that package.json has no "type".
...
const REF_MANIFEST = path.join(ROOT, 'data', 'ai-references.json');

const DIRS = {
  approvedArt: path.join(ROOT, 'assets', 'portraits', 'gods-of-ai'),
  workingArt: path.join(ROOT, 'assets', 'portraits-art', 'gods-of-ai'),
  html: path.join(ROOT, 'dist', 'gods', 'html'),
  enhanced: path.join(ROOT, 'dist', 'gods', 'enhanced'),
  published: path.join(ROOT, 'public', 'cards', 'gods'),
};
...
function likenessPrompt(h) {
  const motif = (h.domains || []).slice(0, 2).join(' and ') || 'machine learning';
  return [
  ...
  if (without.length) {
    console.log(
      `portraits: ${without.length} have no verified reference photo and will get no face ` +
        `(${without.map((h) => h.number).join(', ')}). Run scripts/gods-refs.mjs to look again.`
    );
  }
  ...
  if (!process.env.ENHANCE_I_WILL_CHECK_EVERY_FACE) {
    console.error(
      'enhance: refusing to run.\n' +
        ...
        '  face in dist/gods/enhanced/ before publish.'
    );
    process.exit(1);
  }
  await ensureDirs();
...
async function writeArtPaths(published) {
  ...
  const file = path.join(ROOT, 'src', 'data', 'ai.ts');
  ...
  for (const line of lines) {
    ...
    if (rewriting && !inserted.has(current) && /^ {4}status: '.*',$/.test(line)) {
      ...
      const stem = `/cards/gods/${String(h.number).padStart(3, '0')}-${h.slug}`;
  ...
  console.log(`publish: linked art paths for ${changed} card(s) in src/data/ai.ts`);
}
...
async function validate(roster) {
  ...
  for (const h of roster) {
    ...
    for (const stat of ['impact', 'research', 'systems', 'openness', 'influence']) {
      const v = h[stat];
...
const HELP = `Security Professionals (Series Three) card pipeline
  ...
  node scripts/gods-cards.mjs <command> [card numbers...]
...
Commands
  ...
  publish    Copy the best available faces into public/cards/gods/
  all        portraits -> render -> publish
