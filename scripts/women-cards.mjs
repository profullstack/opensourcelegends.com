#!/usr/bin/env node
/*
  Women in Tech (Series Five) — end-to-end card pipeline.

  Same shape as scripts/security-pros.mjs. Every stage skips work that already
  exists, so re-running is cheap and a failed batch can just be run again.

    portraits  AI portrait art, one per professional -> assets/portraits-art/women-in-tech/
    render     HTML template -> exact card PNGs      -> dist/women/html/
    enhance    image-to-image premium finish pass    -> dist/women/enhanced/  (OPT-IN, see below)
    publish    pick the best face available          -> public/cards/women/
  ...
  scripts/women-refs.mjs), and a card with no verified photo gets NO FACE — the
  ...
  conditioned portrait is a derivative work. data/women-references.json carries
  the provenance and the card page prints it.

  The roster is src/data/women.ts, imported directly (Node strips the types),
  so there is no second copy of the data to keep in sync.

  Portrait resolution order, highest authority first:
    1. assets/portraits/women-in-tech/card_NNN.png   (human-approved, committed)
    2. assets/portraits-art/women-in-tech/card_NNN.png (model output, gitignored)
  ...
  Run:
    node scripts/women-cards.mjs all              # whole set
    node scripts/women-cards.mjs all 1 5 12       # only these numbers
    node scripts/women-cards.mjs render           # no API calls, no spend
*/
...
import { buildFront, buildBack, CARD_W, CARD_H } from './women-template.mjs';
...
// Importing src/data/women.ts makes Node warn that package.json has no "type".
...
const REF_MANIFEST = path.join(ROOT, 'data', 'women-references.json');

const DIRS = {
  approvedArt: path.join(ROOT, 'assets', 'portraits', 'women-in-tech'),
  workingArt: path.join(ROOT, 'assets', 'portraits-art', 'women-in-tech'),
  html: path.join(ROOT, 'dist', 'women', 'html'),
  enhanced: path.join(ROOT, 'dist', 'women', 'enhanced'),
  published: path.join(ROOT, 'public', 'cards', 'women'),
};
...
function likenessPrompt(h) {
  const motif = (h.domains || []).slice(0, 2).join(' and ') || 'computing';
  return [
  ...
  if (without.length) {
    console.log(
      `portraits: ${without.length} have no verified reference photo and will get no face ` +
        `(${without.map((h) => h.number).join(', ')}). Run scripts/women-refs.mjs to look again.`
    );
  }
  ...
  if (!process.env.ENHANCE_I_WILL_CHECK_EVERY_FACE) {
    console.error(
      'enhance: refusing to run.\n' +
        ...
        '  face in dist/women/enhanced/ before publish.'
    );
    process.exit(1);
  }
  await ensureDirs();
...
async function writeArtPaths(published) {
  ...
  const file = path.join(ROOT, 'src', 'data', 'women.ts');
  ...
  for (const line of lines) {
    ...
    if (rewriting && !inserted.has(current) && /^ {4}status: '.*',$/.test(line)) {
      ...
      const stem = `/cards/women/${String(h.number).padStart(3, '0')}-${h.slug}`;
  ...
  console.log(`publish: linked art paths for ${changed} card(s) in src/data/women.ts`);
}
...
async function validate(roster) {
  ...
  for (const h of roster) {
    ...
    for (const stat of ['impact', 'technical', 'pioneering', 'openness', 'influence']) {
      const v = h[stat];
...
const HELP = `Security Professionals (Series Three) card pipeline
  ...
  node scripts/women-cards.mjs <command> [card numbers...]
...
Commands
  ...
  publish    Copy the best available faces into public/cards/women/
  all        portraits -> render -> publish
