#!/usr/bin/env node
/*
  Tech CEOs (Series Six) — end-to-end card pipeline.
  ...
  exists, so re-running is cheap and a failed batch can just be run again.
    ...
    portraits  AI portrait art, one per professional -> assets/portraits-art/tech-ceos/
    render     HTML template -> exact card PNGs      -> dist/ceos/html/
    enhance    image-to-image premium finish pass    -> dist/ceos/enhanced/  (OPT-IN, see below)
    publish    pick the best face available          -> public/cards/ceos/
  ...
  scripts/ceo-refs.mjs), and a card with no verified photo gets NO FACE — the
  ...
  conditioned portrait is a derivative work. data/ceo-references.json carries
  the provenance and the card page prints it.

  The roster is src/data/ceos.ts, imported directly (Node strips the types),
  so there is no second copy of the data to keep in sync.

  Portrait resolution order, highest authority first:
    1. assets/portraits/tech-ceos/card_NNN.png   (human-approved, committed)
    2. assets/portraits-art/tech-ceos/card_NNN.png (model output, gitignored)
  ...
  Run:
    node scripts/ceo-cards.mjs all              # whole set
    node scripts/ceo-cards.mjs all 1 5 12       # only these numbers
    node scripts/ceo-cards.mjs render           # no API calls, no spend
*/
...
import { buildFront, buildBack, CARD_W, CARD_H } from './ceo-template.mjs';
...
// Importing src/data/ceos.ts makes Node warn that package.json has no "type".
...
const REF_MANIFEST = path.join(ROOT, 'data', 'ceo-references.json');

const DIRS = {
  approvedArt: path.join(ROOT, 'assets', 'portraits', 'tech-ceos'),
  workingArt: path.join(ROOT, 'assets', 'portraits-art', 'tech-ceos'),
  html: path.join(ROOT, 'dist', 'ceos', 'html'),
  enhanced: path.join(ROOT, 'dist', 'ceos', 'enhanced'),
  published: path.join(ROOT, 'public', 'cards', 'ceos'),
};
  ...
  if (without.length) {
    console.log(
      `portraits: ${without.length} have no verified reference photo and will get no face ` +
        `(${without.map((h) => h.number).join(', ')}). Run scripts/ceo-refs.mjs to look again.`
    );
  }
  ...
  if (!process.env.ENHANCE_I_WILL_CHECK_EVERY_FACE) {
    console.error(
      'enhance: refusing to run.\n' +
        ...
        '  face in dist/ceos/enhanced/ before publish.'
    );
    process.exit(1);
  }
  await ensureDirs();
...
async function writeArtPaths(published) {
  ...
  const file = path.join(ROOT, 'src', 'data', 'ceos.ts');
  ...
  for (const line of lines) {
    ...
    if (rewriting && !inserted.has(current) && /^ {4}status: '.*',$/.test(line)) {
      ...
      const stem = `/cards/ceos/${String(h.number).padStart(3, '0')}-${h.slug}`;
  ...
  console.log(`publish: linked art paths for ${changed} card(s) in src/data/ceos.ts`);
}
...
async function validate(roster) {
  ...
  for (const h of roster) {
    ...
    for (const stat of ['impact', 'product', 'operating', 'openness', 'influence']) {
      const v = h[stat];
...
const HELP = `Security Professionals (Series Three) card pipeline
  ...
  node scripts/ceo-cards.mjs <command> [card numbers...]
...
Commands
  ...
  publish    Copy the best available faces into public/cards/ceos/
  all        portraits -> render -> publish
