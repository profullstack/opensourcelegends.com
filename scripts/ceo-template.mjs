// HTML/CSS card template for "Tech CEOs" (Series Six).
// Same bones as the other five decks so the whole product line reads as one thing:
// thin accent border, portrait-top front, cream panels, dark skill stack on the back.
// What differs is the axes. Rating a chief executive on technical depth mostly measures
// how long ago they stopped writing code, so this set rates the job: how close they were
// to the product, how well they ran the company, how much it gave away, and what moved.
// Only raster image = the portrait. Everything else is HTML/CSS/SVG.
//
// Carried over from the Series Two defects:
//   1. The curator's note is a labelled panel, NOT wrapped in quote marks. Quoted, it
//      reads as something the subject said, and the field is editorial voice.
//   2. No undecorated empty space on either face, because the image-to-image finish
//      pass treats a blank region as an invitation to invent text. (This set does not
//      run that pass at all.)

export const CARD_W = 700;
export const CARD_H = 1043;

// Distinct from the other five decks without being cute about it. Series Six leans
// metallic — boardroom colours, which is the joke and also just legible on black.
const PALETTE = ['c9a227', '6f8fae', 'b9723f', '4c8b7b', '8f7bd1', 'c05a52', '5d8f4e', 'a8894b'];
...
function traits(h) {
  const m = [
    ['PRODUCT', h.product],
    ['OPERATOR', h.operating],
    ['OPEN SOURCE', h.openness],
...
// Series Six mark: the same shield as the other decks, holding three rising bars.
const emblem = (a, s = 24) =>
  `<svg width="${s}" height="${s}" viewBox="0 0 48 48" fill="none"><path d="M24 3l16 5v13c0 12-7.4 20-16 24C15.4 41 8 33 8 21V8l16-5z" fill="#0a0810" stroke="${a}" stroke-width="2.6"/><rect x="15" y="25" width="4.4" height="7" rx="1.2" fill="${a}"/><rect x="21.8" y="20" width="4.4" height="12" rx="1.2" fill="${a}"/><rect x="28.6" y="14" width="4.4" height="18" rx="1.2" fill="${a}"/></svg>`;
...
export function buildFront(pro, portraitDataUri, ref) {
  ...
  /* 72%, not the Series Two 58%, which left a dead third of black under the caption. */
  ...
  <div class="card">
    ...
    <div class="crestbox"><span class="crest">${emblem(accent, 24)}<span class="t">TECH<br>CEOS</span></span></div>
