// HTML/CSS card template for "Women in Tech" (Series Five).
// Same bones as the other four decks so the whole product line reads as one thing:
// thin accent border, portrait-top front, cream panels, dark skill stack on the back.
// What differs is the axes. This set spans 1815 to now across wildly different
// disciplines, so it rates depth, whether they went first, how much they gave away,
...
// Distinct from the other four decks without being cute about it.
const PALETTE = ['c86fa8', '5b8cf0', 'd9a13a', '3fae96', '9b6bff', 'd4614f', '4f9f6a', 'b07ad6'];
...
// Series Five mark: the same shield as the other decks, holding a three-node graph.
const emblem = (a, s = 24) =>
  `<svg width="${s}" height="${s}" viewBox="0 0 48 48" fill="none"><path d="M24 3l16 5v13c0 12-7.4 20-16 24C15.4 41 8 33 8 21V8l16-5z" fill="#0a0810" stroke="${a}" stroke-width="2.6"/><circle cx="24" cy="15" r="2.8" fill="${a}"/><circle cx="17" cy="26" r="2.8" fill="${a}"/><circle cx="31" cy="26" r="2.8" fill="${a}"/><path d="M24 17.8L17.8 23.4M24 17.8l6.2 5.6M18.6 28.2h10.8" stroke="${a}" stroke-width="2.1" stroke-linecap="round"/></svg>`;
...
export function buildFront(pro, portraitDataUri, ref) {
  ...
  <div class="card">
    ...
    <div class="crestbox"><span class="crest">${emblem(accent, 24)}<span class="t">WOMEN<br>IN TECH</span></span></div>
